...
- `src/features/canvas/models/image/*` - 各供应商模型定义目录
- `src/features/canvas/models/image/volcano` - 火山引擎模型
- `src/features/canvas/models/image/volcanoVision` - 火山方舟视觉模型（Seedream 4.5）

#### 默认模型配置
- 智能分镜节点（SmartStoryboardNode）：`volcano/ep-20260410002744-29gfm`
- 上传图片节点（UploadNode）：`volcano/ep-20260410002744-29gfm`
- 分镜生成节点（StoryboardGenNode）：由用户选择

#### AI 图片处理规范
- 大图分析时自动压缩：`compressImageForAIAnalysis()` 函数，将图片压缩到 3500 万像素以内
- 参考图处理：根据不同供应商使用不同格式
  - KIE/FAL：base64 格式
  - 火山引擎：base64 格式（自动压缩）
  - 其他供应商：本地文件路径
...
#### 前端供应商目录
每个供应商在 `src/features/canvas/models/providers/` 下有独立的定义文件：
- `volcano.ts` - 火山引擎（图片分析）
- `volcanoVision.ts` - 火山方舟视觉（Seedream 4.5 图片生成）

#### 前端模型目录
每个模型在 `src/features/canvas/models/image/<provider>/` 下有独立文件：
- `volcano/doubaoSeedV2Lite.ts`
- `volcano/ep.ts`
- `volcanoVision/seedream45.ts`

#### 后端 Rust 供应商
- `src-tauri/src/ai/providers/volcano/mod.rs` - 火山引擎（分析+生成）
- 注意：volcano-vision 在后端映射到 volcano 供应商，通过 set_api_key 命令的映射逻辑处理
- 后端命令 `set_api_key`、`generate_image`、`analyze_image` 都需要处理 provider 映射
