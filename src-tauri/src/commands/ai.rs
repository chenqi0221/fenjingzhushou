...
pub async fn set_api_key(provider: String, api_key: String) -> Result<(), String> {
    ...
    info!("Setting API key for provider: {}, api_key prefix: '{}'", provider, key_prefix);

    // 保持火山方舟视觉提供商的独立性
    let resolved_provider_id = provider.clone();

    let registry = get_registry();
    let resolved_provider = registry
        .get_provider(resolved_provider_id.as_str())
...
pub async fn generate_image(request: GenerateRequestDto) -> Result<String, String> {
    ...
    // 使用原始模型ID进行提供商查找
    let model_for_lookup = request.model.clone();
    
    let provider = registry
        .resolve_provider_for_model(&model_for_lookup)
        .or_else(|| registry.get_default_provider())
        .ok_or_else(|| "Provider not found".to_string())?;
...
#[tauri::command]
pub async fn analyze_image(image_url: String, model: Option<String>, prompt: Option<String>) -> Result<String, String> {
    info!("analyze_image called: model={:?}, prompt={:?}", model, prompt);
    ...
    info!("Resolving provider: {}, model: {}", provider_id, analysis_model);
    ...
    let analysis_prompt = prompt.unwrap_or_else(|| "请分析这张图片，描述其内容和主要元素。".to_string());

    provider.analyze_image(&image_url, analysis_model, &analysis_prompt).await.map_err(|e| e.to_string())
}
...
pub async fn chat_completion(
    ...
    info!("Chat completion with model: {:?}", model);
    ...
    let opts = options.unwrap_or(ChatCompletionOptions {
        temperature: 0.7,
        max_tokens: None,
    });

    let messages_for_model: Vec<crate::ai::ChatMessage> = messages
        .into_iter()
