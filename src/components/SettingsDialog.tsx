...
export function SettingsDialog({
  ...
  const handleSave = useCallback(async () => {
    // 先将 API key 保存到 store，然后同步到后端
    for (const provider of providers) {
      const apiKey = localApiKeys[provider.id] ?? '';
      ...
      if (apiKey) {
        try {
          ...
          console.warn(`Failed to set API key for ${provider.id}:`, e);
        }
      }
    }
  ...
  const handleCheckUpdate = useCallback(async () => {
    ...
    // 转换状态值以匹配类型
    let convertedStatus: '' | 'checking' | 'no-update' | 'update-available' | 'error';
    switch (status) {
      case 'has-update':
        convertedStatus = 'update-available';
        break;
      case 'up-to-date':
        convertedStatus = 'no-update';
        break;
      case 'failed':
    ...
    setCheckUpdateStatus(convertedStatus);
  }, [onCheckUpdate]);
                      ...
                      <div key={provider.id} className="rounded-lg border border-border-dark bg-bg-dark p-4">
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full rounded border border-border-dark bg-surface-dark px-3 py-2 pr-32 text-sm text-text-dark placeholder:text-text-muted"
                          />
                          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
                            <button
                              ...
                              className="text-xs px-2 py-1 bg-bg-dark rounded border border-border-dark hover:border-accent transition-colors"
                            >
                              测试连接
                            </button>
                            <button
                ...
                <div className="ui-scrollbar flex-1 space-y-4 overflow-y-auto p-6">
                  ...
                  <div className="rounded-lg border border-border-dark bg-bg-dark p-4">
                    <div className="mb-3">
                      ...
                      <p className="mt-1 text-xs text-text-muted">
                        设置用于图像分析的提示词，AI 会根据这个提示词分析图片内容。
                      </p>
                    </div>
                    ...
                    <textarea
                      ...
                      placeholder="请分析这张图片，描述其内容和主要元素。"
                      className="w-full rounded border border-border-dark bg-surface-dark px-3 py-2 text-sm text-text-dark outline-none placeholder:text-text-muted resize-none"
                      rows={4}
                    />
                  </div>
                    ...
                    <div className="pt-1">
                      ...
                      {checkUpdateStatus !== '' && (
                        <p className="mt-2 text-xs text-text-muted">
                          {checkUpdateStatus === 'update-available' && t('settings.checkUpdateHasUpdate')}
                          {checkUpdateStatus === 'no-update' && t('settings.checkUpdateUpToDate')}
                          {checkUpdateStatus === 'error' && t('settings.checkUpdateFailed')}
