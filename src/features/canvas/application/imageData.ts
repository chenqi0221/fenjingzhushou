...
const AI_ANALYSIS_MAX_PIXELS = 35000000;

export async function compressImageForAIAnalysis(dataUrl: string): Promise<string> {
  const image = await loadImageElement(dataUrl);
  const totalPixels = image.naturalWidth * image.naturalHeight;
  
  if (totalPixels <= AI_ANALYSIS_MAX_PIXELS) {
    return dataUrl;
  }
  
  const scale = Math.sqrt(AI_ANALYSIS_MAX_PIXELS / totalPixels);
