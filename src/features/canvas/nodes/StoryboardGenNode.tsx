...
export const StoryboardGenNode = memo(({ id, data, selected, width, height }: StoryboardGenNodeProps) => {
  ...
  const handleGenerate = useCallback(async (previewGridOnly = false) => {
    ...
    try {
      ...
      // 收集所有分镜中 @ 引用的图片索引
      const referencedImageIndices = new Set<number>();
      nodeData.frames.forEach((frame) => {
        if (frame.referenceIndex !== null) {
          referencedImageIndices.add(frame.referenceIndex);
        }
      });

      // 构建参考图片列表：只包含被 @ 引用的图片 + 网格图片
      if (referencedImageIndices.size > 0) {
        // 只包含被引用的图片
        const referencedImages = Array.from(referencedImageIndices)
          .filter(index => index >= 0 && index < incomingImages.length)
          .map(index => incomingImages[index]);
        allReferenceImages = [...referencedImages, gridImageDataUrl];
      } else {
        // 没有引用时使用所有上游图片
        allReferenceImages = [...incomingImages, gridImageDataUrl];
      }
      ...
      const safeReferenceImageCount = incomingImages.length + 1;
      const generationDebugContext: GenerationDebugContext = {
        sourceType: 'storyboardGen',
        providerId: selectedModel.providerId,
        requestModel: requestResolution.requestModel,
        ...
        prompt,
        extraParams: effectiveExtraParams,
        referenceImageCount: safeReferenceImageCount,
        referenceImagePlaceholders: createReferenceImagePlaceholders(safeReferenceImageCount),
        appVersion: runtimeDiagnostics.appVersion,
        osName: runtimeDiagnostics.osName,
        osVersion: runtimeDiagnostics.osVersion,
        osBuild: runtimeDiagnostics.osBuild,
  ...
  return (
    ...
    >
      ...
      {/* Controls */}
      <div
        className="relative mx-auto mt-auto flex shrink-0 flex-col gap-2"
        style={{ width: `${frameLayout.paramsRowWidth}px` }}
      >
        {/* AI Parameters and Generate Button */}
